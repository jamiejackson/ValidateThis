...
<cfcomponent extends="validatethis.unitTests.SRV.BaseForServerRuleValidatorTests" output="false">
	...
	<cffunction name="setUp" access="public" returntype="void">
		<cfscript>
			super.setup();
			SRV = getSRV("Extension");
			parameters = {extension="txt|pdf"};
		</cfscript>
	</cffunction>
	
	<cffunction name="validateReturnsFalseForInvalidExtension" access="public" returntype="void">
		<cfscript>
			objectValue = 1;
            configureValidationMock();
            var fileUploadUtilMock = mock(SRV.getFileUploadUtil());
            fileUploadUtilMock.getUploadFileName("{string}").returns("fakeFile.exe");
            fileUploadUtilMock.getFileExtension("{string}").returns("exe");
			executeValidate(validation);
			validation.verifyTimes(1).fail("{*}"); 
		</cfscript>  
	</cffunction>
	
	<cffunction name="validateReturnsTrueForValidExtension" access="public" returntype="void">
		<cfscript>
			objectValue = 1;
            configureValidationMock();
            var fileUploadUtilMock = mock(SRV.getFileUploadUtil());
            fileUploadUtilMock.getUploadFileName("{string}").returns("fakeFile.pdf");
            fileUploadUtilMock.getFileExtension("{string}").returns("pdf");
			executeValidate(validation);
